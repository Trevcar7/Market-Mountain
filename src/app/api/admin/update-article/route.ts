 ...
 * Patch specific fields on an existing news item in the feed.
 *
 * Auth: Bearer token matching FETCH_NEWS_SECRET
 *
 * Body:
 *   { "id": "news-xxx", "fields": { ...partial NewsItem fields } }
 *
 * Response:
 *   { success, updated: { id, title } }
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const updatedCollection: NewsCollection = {
      ...newsData,
      lastUpdated: new Date().toISOString(),
      news: newNews,
    };

    await kv.set("news", updatedCollection);

    console.log(`[update-article] Patched article ${id} with fields: ${Object.keys(fields).join(", ")}`);
    ...
    console.error("[update-article] KV error:", err);
    return NextResponse.json(
