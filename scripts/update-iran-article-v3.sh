#!/bin/bash
# Update the Iran/oil article: uppercase chartLabels (ENERGY MARKETS, MARKET CONTEXT).
# The story sentence fix (semicolon) must be applied separately if the story field needs patching.
# Usage: FETCH_NEWS_SECRET=your_secret bash scripts/update-iran-article-v3.sh
...
if [ -z "$TOKEN" ]; then
  ...
  echo "Usage: FETCH_NEWS_SECRET=your_secret bash scripts/update-iran-article-v3.sh"
  exit 1
fi
